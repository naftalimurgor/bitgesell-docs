...
{                                         (json object)
  ...
  "paytxfee" : n,                         (numeric) the transaction fee configuration, set in BGL/kvB
...
```sh
BGL-cli getwalletinfo
```

```sh
curl --user myusername --data-binary '{"jsonrpc": "1.0", "id": "curltest", "method": "getwalletinfo", "params": []}' -H 'content-type: text/plain;' http://127.0.0.1:8334/
```
